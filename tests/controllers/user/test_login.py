...
@pytest.mark.asyncio
class TestUserLoginAPIController(TestIUserController):

    @pytest.fixture
    def valid_user_login_request_dto(
        self,
        reference_number,
    ):
    ...
    @pytest.fixture
    def mock_user_login_service(self):
        """Create a mock user login service."""
        service = Mock()
        service.run = AsyncMock()
        return service

    @pytest.fixture
    def mock_user_login_service_factory(self, mock_user_login_service):
        """Create a mock user login service factory."""
        factory = Mock()
        factory.return_value = mock_user_login_service
        return factory
    ...
    async def test_user_login_api_controller_success(
        self,
        valid_user_login_request_dto,
        mock_request,
        mock_session,
        mock_user_repository_factory,
        mock_user_login_service_factory,
        ...
        mock_user_login_service_factory.return_value = mock_service

        response = await controller.post(
            request=mock_request,
            request_payload=valid_user_login_request_dto,
            session=mock_session,
            user_repository=mock_user_repository_factory,
            user_login_service_factory=mock_user_login_service_factory,
    ...
    async def test_user_login_api_controller_invalid_email(
        self,
        reference_number,
        invalid_email,
    ):
    ...
    @pytest.mark.parametrize(
        "invalid_password", ["", "weak", "123", "password"]
    )
    async def test_user_login_api_controller_invalid_password(
        self,
        reference_number,
        invalid_password,
    ):
    ...
    async def test_user_login_api_controller_bad_input_error(
        self,
        valid_user_login_request_dto,
        mock_request,
        mock_session,
        ...
        mock_user_login_service_factory.return_value.run = AsyncMock(
    ...
    async def test_user_login_api_controller_not_found_error(
        self,
        valid_user_login_request_dto,
        mock_request,
        mock_session,
        ...
        response_content = response.body.decode()
        assert "User not found" in response_content
        assert "error_not_found" in response_content

    async def test_user_login_api_controller_unexpected_response_error(
        self,
        valid_user_login_request_dto,
        mock_request,
        mock_session,
    ...
    async def test_user_login_api_controller_generic_exception(
        self,
        valid_user_login_request_dto,
        mock_request,
        mock_session,
        ...
        response_content = response.body.decode()
        assert "Failed to login user" in response_content
        assert "error_internal_server_error" in response_content

    async def test_user_login_api_controller_dictionary_utility_called(
        self,
        valid_user_login_request_dto,
        mock_request,
        mock_session,
    ...
    async def test_service_factory_called_with_correct_params(
        ...
        mock_user_login_service_factory.assert_called_once_with(
            urn=urn,
